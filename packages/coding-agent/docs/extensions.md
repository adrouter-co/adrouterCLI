...
| Package | Purpose |
|---------|---------|
| `@adrouter/cli` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@adrouter/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@adrouter/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@adrouter/cli";
...
```typescript
import { isBashToolResult } from "@adrouter/cli";
...
```typescript
import { createLocalBashOperations } from "@adrouter/cli";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@adrouter/cli";
...
```typescript
import { SessionManager } from "@adrouter/cli";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@adrouter/ai";
...
```typescript
import type { AutocompleteItem } from "@adrouter/tui";
...
```typescript
import { Box, Text } from "@adrouter/tui";
...
```typescript
import { withFileMutationQueue } from "@adrouter/cli";
...
**Important:** Use `StringEnum` from `@adrouter/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@adrouter/cli";
...
```typescript
import { createBashTool } from "@adrouter/cli";
...
```typescript
import { keyHint } from "@adrouter/cli";
...
```typescript
import { Text, Component } from "@adrouter/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@adrouter/cli";
import { matchesKey } from "@adrouter/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@adrouter/cli";
