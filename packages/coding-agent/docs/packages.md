...
Package and CLI updates are disabled for the bundled AdRouterCLI launch. Install
a different package spec explicitly when external replacement is required.
...
- Versioned specs remain pinned until explicitly replaced with another install source.
...
- Refs are pinned tags or commits and do not move until another source or ref is installed explicitly.
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@adrouter/ai`, `@adrouter/agent-core`, `@adrouter/cli`, `@adrouter/tui`, `typebox`.
