...
Updates are disabled for this bundled AdRouterCLI launch. The remaining package
commands manage user-installed packages; `pi config` and project package
commands accept `--approve`/`--no-approve` to trust or ignore project-local
