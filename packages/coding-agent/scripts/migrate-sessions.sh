#!/usr/bin/env bash
#
# Migrate sessions from ~/.adrouter/agent/*.jsonl to proper session directories.
# This fixes sessions created by the bug in v0.30.0 where sessions were
# saved to ~/.adrouter/agent/ instead of ~/.adrouter/agent/sessions/<encoded-cwd>/.
#
...
AGENT_DIR="${ADROUTER_CODING_AGENT_DIR:-$HOME/.adrouter/agent}"
DRY_RUN=false
