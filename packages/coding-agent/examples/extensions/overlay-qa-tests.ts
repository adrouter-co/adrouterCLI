...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@adrouter/cli";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@adrouter/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@adrouter/tui";
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/tmp/adrouter-overlay-qa/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /tmp/adrouter-overlay-qa\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
