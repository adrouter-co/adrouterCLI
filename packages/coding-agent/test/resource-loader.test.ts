...
describe("DefaultResourceLoader", () => {
	...
	describe("reload", () => {
		...
		it("loads bundled commands, web tools, and skills without diagnostics", async () => {
			...
			expect([...webExtension!.tools.keys()].sort()).toEqual(["fetch_content", "get_search_content", "web_search"]);
			expect([...webExtension!.commands.keys()].sort()).toEqual([
				"curator",
				"google-account",
				"search",
				"websearch",
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".adrouter", "prompts");
			...
			const projectSkillDir = join(cwd, ".adrouter", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".adrouter", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".adrouter", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".adrouter"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".adrouter", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".adrouter", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".adrouter", "extensions");
...
export default function(pi) {
	...
	pi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".adrouter", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.adrouter", async () => {
			const piDir = join(cwd, ".adrouter");
