...
"[Extensions]
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const externalRoot = path.resolve(path.sep, "external-adrouter-home");
		const cwd = path.resolve(path.sep, "workspace", "project");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(externalRoot, ".adrouter", "agent", "AGENTS.md") },
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain(`${externalRoot.replace(/\\/g, "/")}/.adrouter/agent/AGENTS.md`);
		expect(output).toContain(`${cwd.replace(/\\/g, "/")}/AGENTS.md`);
		expect(output).not.toContain(`${externalRoot.replace(/\\/g, "/")}/.adrouter/agent/AGENTS.md, AGENTS.md`);
	});
