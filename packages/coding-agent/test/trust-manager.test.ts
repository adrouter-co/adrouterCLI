...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".adrouter", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".adrouter", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".adrouter", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".adrouter"), { recursive: true });
			writeFileSync(join(cwd, ".adrouter", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".adrouter"), { recursive: true, force: true });
