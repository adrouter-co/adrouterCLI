...
Content`,
		...
		it("should auto-discover root markdown skills from .adrouter skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .adrouter", async () => {
			const extDir = join(tempDir, ".adrouter", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".adrouter"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".adrouter", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".adrouter", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".adrouter", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".adrouter", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".adrouter", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .adrouter/agent skills", async () => {
		...
		it("should use the project .adrouter dir as baseDir for project .adrouter skills", async () => {
			const projectBaseDir = join(tempDir, ".adrouter");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".adrouter", "agent");
		...
		it("should dedupe user skill entries when ~/.adrouter/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .adrouter auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".adrouter\n");

			const skillDir = join(tempDir, ".adrouter", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.adrouter\\npm";
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .adrouter settings base", () => {
			...
			const rel = relative(join(tempDir, ".adrouter"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{
					source: relative(join(tempDir, ".adrouter"), pkgDir),
					autoload: false,
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should not run npm view during resolve for installed unpinned packages", async () => {
			process.env.PI_OFFLINE = "1";
			const installedPath = join(tempDir, ".adrouter", "npm", "node_modules", "example");
			mkdirSync(join(installedPath, "extensions"), { recursive: true });
