...
describe("find relativizes root search paths", () => {
	...
	it("relativizes custom glob results through the find tool", async () => {
		const definition = createFindToolDefinition("/", {
			operations: {
				exists: () => true,
		...
		const result = (await definition.execute("call-1", { pattern: "**" }, undefined, undefined, context)) as {
