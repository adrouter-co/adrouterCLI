...
import type { BuildSystemPromptOptions, ExtensionAPI, ExtensionContext } from "@adrouter/cli";
...
const STATE_DIR = process.env.ADROUTER_CODING_AGENT_DIR || join(homedir(), ".adrouter", "agent");
...
/**
 * Return a platform-friendly display path for `~/.adrouter/agent/models.json`.
 *
 * On Windows (platform starts with "win") the path is shown as
 * `%USERPROFILE%\.adrouter\agent\models.json` to match Windows conventions.
 * On all other platforms (Linux, macOS, etc.) it is shown as
 * `~/.adrouter/agent/models.json` (the Unix-style tilde shorthand).
 *
...
function getModelsJsonDisplayPath(platform: string = process.platform): string {
  if (platform.startsWith("win")) {
    return `%USERPROFILE%\\.adrouter\\agent\\models.json`;
  }
  return "~/.adrouter/agent/models.json";
}
 ...
 * state file at `~/.adrouter/agent/pi-cache-optimizer-stats.json`.
 */
