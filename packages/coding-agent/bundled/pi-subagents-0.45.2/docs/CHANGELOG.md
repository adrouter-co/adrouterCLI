...
- Advanced provenance to the reviewed upstream 0.45.2 source while retaining only the bounded
  single, parallel, chain, status, interrupt, resume, stop, and doctor subset.
- Disabled nested delegation, managed worktrees, Gist sharing, external intercom, schedules,
  missions, profiles, watchdogs, and arbitrary workflow products.
- Added a portable private-file stop request and an `ADROUTER_SUBAGENTS=off` kill switch.
