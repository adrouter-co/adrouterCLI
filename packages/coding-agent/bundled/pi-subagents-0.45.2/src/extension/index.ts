 ...
 *
 * Config file: ~/.adrouter/agent/extensions/subagent/config.json
 * AdRouter policy always caps depth/concurrency and disables intercom/worktree hooks.
 */

import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import type { AgentToolResult } from "@adrouter/agent-core";
import { type ExtensionAPI, type ExtensionContext, type ToolDefinition } from "@adrouter/cli";
import { Box, Container, Spacer, Text, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component } from "@adrouter/tui";
...
const ADROUTER_SUBAGENTS_ENV = "ADROUTER_SUBAGENTS";
...
function registerDisabledSubagentSurface(pi: ExtensionAPI): void {
	const disabledMessage = `Subagents are disabled by ${ADROUTER_SUBAGENTS_ENV}=off.`;
	const tool: ToolDefinition<typeof SubagentParams, Details> = {
		...
		async execute() {
			return {
				content: [{ type: "text", text: disabledMessage }],
				isError: true,
	...
	for (const name of DISABLED_COMMANDS) {
		pi.registerCommand(name, {
			description: "Disabled AdRouter subagent command",
			handler: async (_args, ctx) => ctx.ui.notify(disabledMessage, "warning"),
		});
	}
	pi.on("session_start", () => {});
	pi.on("tool_result", () => {});
...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	if (process.env[ADROUTER_SUBAGENTS_ENV]?.trim().toLowerCase() === "off") {
		registerDisabledSubagentSurface(pi);
		return;
	}
...
• PARALLEL: { tasks: [{agent,task,count?,output?,reads?,progress?}, ...], concurrency?: number } - at most three children, with at most one mutation-capable profile
...
• { action: "models" } - list available model identifiers without provider credentials
• { action: "children" } - inspect child runs owned by the active session
• Use chainName with get for read-only chain inspection; packaged chains use dotted runtime names

CONTROL:
• { action: "status", id: "..." } - inspect an async/background run by id or prefix
• { action: "interrupt", id?: "..." } - soft-interrupt the current child turn and leave the run paused
• { action: "stop", id?: "..." } - stop a foreground or background run without targeting an unverified PID
...
OUT OF SCOPE:
• No agent/chain mutation, dynamic fanout, executable acceptance commands, agent-supplied extensions, missions, schedules, profiles, watchdogs, managed worktrees, Gist sharing, external intercom, or nested delegation.`,
		parameters: SubagentParams,
		effect: "mutation",
