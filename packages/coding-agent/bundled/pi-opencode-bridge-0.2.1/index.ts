...
const ADROUTER_AGENT_DIR = process.env.ADROUTER_CODING_AGENT_DIR || join(homedir(), ".adrouter", "agent");
const AUTH_PATHS = [
	join(homedir(), ".local", "share", "opencode", "auth.json"),
	join(ADROUTER_AGENT_DIR, "auth.json"),
];
const OPENCODE_AUTH_PATH = join(homedir(), ".local", "share", "opencode", "auth.json");
const ADROUTER_AUTH_PATH = join(ADROUTER_AGENT_DIR, "auth.json");
...
function setOpencodeGoApiKey(apiKey: string): void {
	...
	const adrouterDir = ADROUTER_AGENT_DIR;
	...
	// Also sync to AdRouterCLI's auth.json so its AuthStorage picks it up.
	if (!existsSync(adrouterDir)) {
		mkdirSync(adrouterDir, { recursive: true, mode: 0o700 });
	}
	let adrouterData: Record<string, unknown> = {};
	if (existsSync(ADROUTER_AUTH_PATH)) {
		try {
			adrouterData = JSON.parse(readFileSync(ADROUTER_AUTH_PATH, "utf-8"));
		} catch {
			// corrupt but continue — will overwrite
		}
	}
	adrouterData["opencode-go"] = { type: "api_key", key: apiKey };
	writeFileSync(ADROUTER_AUTH_PATH, JSON.stringify(adrouterData, null, 2), { encoding: "utf-8", mode: 0o600 });
	try { chmodSync(ADROUTER_AUTH_PATH, 0o600); } catch { /* best-effort */ }
}
