...
export function parseArgs(args: string[]): Args {
	...
	for (let i = 0; i < args.length; i++) {
		...
		} else if (arg === "--tool-policy" && i + 1 < args.length) {
			result.toolPolicy = args[++i];
...
${chalk.bold("Options:")}
  ...
  --profile <name>               Apply a named profile to project .adrouter state
  ...
  --tool-policy <absolute-path>  Exact one-shot tool approvals for print/JSON mode
