...
import { type ImageContent, modelsAreEqual } from "@adrouter/ai";
import chalk from "chalk";
import { handleAdRouterCommand } from "./cli/adrouter-command.ts";
...
import { applyProfile } from "./core/profiles.ts";
...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "adrouter -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (process.env.ADROUTER_ADS_ENABLED === undefined) {
		process.env.ADROUTER_ADS_ENABLED =
			bootstrapSettingsManager.getGlobalSettings().adsEnabled === false ? "false" : "true";
	}
	applyHttpProxySettings(bootstrapSettingsManager.getGlobalSettings().httpProxy);
	configureHttpDispatcher();
	if (await handleAdRouterCommand(args, cwd)) {
		return;
	}
	...
	if (parsed.profile) {
		try {
			applyProfile(parsed.profile, cwd);
		} catch (error) {
			console.error(chalk.red(`Error: ${error instanceof Error ? error.message : String(error)}`));
			process.exit(1);
		}
	}
	if (parsed.provider === "adrouter" && parsed.model === "deepseek") {
		console.error(chalk.red('Error: model "deepseek" was retired. Use "deepseek-v4-flash" or "deepseek-v4-pro".'));
		process.exit(1);
	}
