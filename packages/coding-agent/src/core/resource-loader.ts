...
export interface DefaultResourceLoaderOptions {
	...
	/** Load AdRouterCLI's vendored extensions and skills. CLI startup enables this explicitly. */
	includeBundledFeatures?: boolean;
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private includeBundledFeatures: boolean;
	...
	constructor(options: DefaultResourceLoaderOptions) {
		...
		this.includeBundledFeatures = options.includeBundledFeatures ?? false;
	...
	async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
		...
		// Source checkouts keep bundled resources at the package root; the Node
		// release copies them into dist alongside the compiled modules.
		...
		const skillPaths = this.noSkills
			...
			: this.mergePaths([...bundledSkills, ...cliEnabledSkills, ...enabledSkills], this.additionalSkillPaths);
	...
	private mapSkillPath(resource: ResolvedResource, metadataByPath: Map<string, PathMetadata>): string {
		...
		const directSkillFile = join(resource.path, "SKILL.md");
		const skillFile = existsSync(directSkillFile) ? directSkillFile : join(resource.path, "docs", "SKILL.md");
