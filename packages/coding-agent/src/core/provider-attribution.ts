import type { Api, Model, ProviderHeaders } from "@adrouter/ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"X-OpenRouter-Title": "AdRouterCLI",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "AdRouterCLI",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "adrouter-cli",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "adrouter" };
}
