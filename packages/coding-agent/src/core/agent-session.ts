...
import { ADROUTER_SETTLEMENT_ENTRY, adRouterSettlementEntryData } from "./adrouter-session.ts";
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				if (event.message.role === "assistant") {
					const data = adRouterSettlementEntryData(getAdRouterMessageUpdate(event.message));
					if (data) {
						const entryId = this.sessionManager.appendCustomEntry(ADROUTER_SETTLEMENT_ENTRY, data);
