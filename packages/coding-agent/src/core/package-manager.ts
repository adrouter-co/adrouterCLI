...
export interface ProgressEvent {
	...
	action: "install" | "remove" | "clone" | "pull";
	source: string;
	message?: string;
}
...
function collectSkillEntries(
	...
	try {
		...
		const skillCandidates = [join(dir, "SKILL.md"), join(dir, "docs", "SKILL.md")];
		for (const fullPath of skillCandidates) {
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside AdRouterCLI and resolve host APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// API peers. Stale auto-installed host peers can otherwise block installation.
	...
	private async refreshGit(source: GitSource, scope: SourceScope): Promise<void> {
		const targetDir = this.getGitInstallPath(source, scope);
		if (!existsSync(targetDir)) {
	...
	private async refreshTemporaryGitSource(source: GitSource, sourceStr: string): Promise<void> {
		...
		try {
			await this.withProgress("pull", sourceStr, `Refreshing ${sourceStr}...`, async () => {
				await this.refreshGit(source, "temporary");
			});
		} catch {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from .adrouter/
			addResources(
				"extensions",
		...
		// User extensions from ~/.adrouter/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.adrouter/agent/
		addResources(
			"skills",
