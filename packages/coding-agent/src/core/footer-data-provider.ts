...
export type FooterDataProviderWatchDependencies = {
	watchDirectory: (path: string, listener: WatchListener<string>, onError: () => void) => FSWatcher | null;
	watchPolling: (
		path: string,
		options: { interval: number },
		listener: (current: Stats, previous: Stats) => void,
	) => unknown;
	unwatchPolling: (path: string, listener: (current: Stats, previous: Stats) => void) => void;
};

const defaultWatchDependencies: FooterDataProviderWatchDependencies = {
	watchDirectory: watchWithErrorHandler,
	watchPolling: (path, options, listener) => watchFile(path, options, listener),
	unwatchPolling: (path, listener) => unwatchFile(path, listener),
};
...
export class FooterDataProvider {
	...
	private reftableTablesListPollingListener: ((current: Stats, previous: Stats) => void) | null = null;
	private watchDependencies: FooterDataProviderWatchDependencies;
	...
	constructor(cwd: string, watchDependencies: Partial<FooterDataProviderWatchDependencies> = {}) {
		this.cwd = cwd;
		this.watchDependencies = { ...defaultWatchDependencies, ...watchDependencies };
	...
	private clearGitWatchers(): void {
		...
		if (this.headWatchFilePath && this.headWatchFileListener) {
			this.watchDependencies.unwatchPolling(this.headWatchFilePath, this.headWatchFileListener);
		...
		if (this.reftableTablesListPath && this.reftableTablesListPollingListener) {
			this.watchDependencies.unwatchPolling(this.reftableTablesListPath, this.reftableTablesListPollingListener);
			this.reftableTablesListPath = null;
			this.reftableTablesListPollingListener = null;
		}
	...
	private setupGitWatcher(): void {
		...
		this.headWatcher = this.watchDependencies.watchDirectory(
		...
		if (pollGitHead) {
			...
			this.watchDependencies.watchPolling(this.headWatchFilePath, { interval: 1000 }, this.headWatchFileListener);
		}
		...
		if (existsSync(reftableDir)) {
			this.reftableWatcher = this.watchDependencies.watchDirectory(
				reftableDir,
				() => {
					this.scheduleRefresh();
				},
			...
			if (existsSync(tablesListPath)) {
				...
				this.reftableTablesListWatcher = this.watchDependencies.watchDirectory(
					tablesListPath,
					() => {
						this.scheduleRefresh();
					},
				...
				this.reftableTablesListPollingListener = (current, previous) => {
					if (
						current.mtimeMs !== previous.mtimeMs ||
						current.ctimeMs !== previous.ctimeMs ||
						current.size !== previous.size
				...
				this.watchDependencies.watchPolling(
					tablesListPath,
					{ interval: 250 },
					this.reftableTablesListPollingListener,
				);
			}
		}
	}
