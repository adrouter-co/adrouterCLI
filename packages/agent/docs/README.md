...
Stateful agent with tool execution and event streaming. Built on `@adrouter/ai`.

## Installation

```bash
npm install @adrouter/agent-core
```

## Quick Start

```typescript
import { Agent } from "@adrouter/agent-core";
...
```typescript
declare module "@adrouter/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@adrouter/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@adrouter/agent-core";
