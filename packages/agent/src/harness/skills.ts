...
export function formatSkillInvocation(skill: Skill, additionalInstructions?: string): string {
	const skillBlock = `<skill name="${skill.name}" location="${skill.filePath}">\nReferences are relative to ${skillRootFromFile(skill.filePath)}.\n\n${skill.content}\n</skill>`;
	return additionalInstructions ? `${skillBlock}\n\n${additionalInstructions}` : skillBlock;
}

function skillRootFromFile(filePath: string): string {
	const skillDir = dirnameEnvPath(filePath);
	return basenameEnvPath(skillDir) === "docs" ? dirnameEnvPath(skillDir) : skillDir;
}
...
async function loadSkillsFromDirInternal(
	...
	const skillPaths = entries.filter((entry) => entry.name === "SKILL.md").map((entry) => entry.path);
	const docsSkillPath = joinEnvPath(joinEnvPath(dir, "docs"), "SKILL.md");
	const docsSkillInfo = await env.fileInfo(docsSkillPath);
	if (docsSkillInfo.ok && docsSkillInfo.value.kind === "file") skillPaths.push(docsSkillPath);
...
async function loadSkillFromFile(
	...
	const skillDir = skillRootFromFile(filePath);
