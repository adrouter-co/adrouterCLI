...
describe("Editor component", () => {
	describe("text selection", () => {
		it("replaces a Shift+Arrow selection when typing", () => {
			...
			assert.strictEqual(editor.getText(), "helX");
		});

		it("copies mouse-selected editor text with OSC 52", () => {
			const terminal = new ClipboardTerminal(30, 10);
			const editor = new Editor(new TUI(terminal), defaultEditorTheme);
			editor.setText("hello");
			editor.render(30);

			editor.beginMouseSelection(1, 1);
			editor.updateMouseSelection(1, 4);
			editor.endMouseSelection(1, 4);
			editor.handleInput("\x1b[99;9u");

			assert.strictEqual(terminal.clipboardWrites.length, 1);
			const payload = terminal.clipboardWrites[0]!.slice("\x1b]52;c;".length, -1);
			assert.strictEqual(Buffer.from(payload, "base64").toString("utf8"), "ell");
		});
	});
