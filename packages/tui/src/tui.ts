...
export interface Component {
	...
	/** Optional TUI-owned mouse selection hooks for fixed bottom components. */
...
type BottomComponentRange = { component: Component; start: number; end: number };
type BottomSelectionTarget = {
	component: Component;
	row: number;
	col: number;
...
export class TUI extends Container {
	...
	private bottomAnchorStartComponent: Component | null = null;
	private transcriptScrollOffset = 0;
	private transcriptMaxScrollOffset = 0;
	private transcriptViewportHeight = 0;
	private transcriptTopLineCount = 0;
	private transcriptVisibleStartLine = 0;
	private transcriptAllLines: string[] = [];
	private bottomViewportStartRow = 0;
	private bottomComponentRanges: BottomComponentRange[] = [];
	private activeBottomSelectionComponent: Component | null = null;
	private activeBottomSelectionTarget: BottomSelectionTarget | null = null;
	private selectionAttention: "transcript" | "bottom-component" | null = null;
	...
	/**
	 * Keep this direct child and every following child fixed at the bottom of
	 * the terminal. Earlier children render as the scrollable transcript.
	 */
	setBottomAnchorStart(component: Component | null): void {
		if (this.bottomAnchorStartComponent === component) return;
		this.stopTranscriptSelectionAutoscroll();
		this.bottomAnchorStartComponent = component;
		this.transcriptScrollOffset = 0;
		this.transcriptMaxScrollOffset = 0;
		this.transcriptViewportHeight = 0;
		this.transcriptTopLineCount = 0;
		this.transcriptVisibleStartLine = 0;
		this.transcriptAllLines = [];
		this.bottomViewportStartRow = 0;
		this.bottomComponentRanges = [];
		this.activeBottomSelectionComponent = null;
		this.activeBottomSelectionTarget = null;
		this.selectionAttention = null;
		this.transcriptSelection = undefined;
		this.requestRender();
	}

	scrollTranscript(deltaLines: number): boolean {
		if (!this.bottomAnchorStartComponent) return false;
		if (!Number.isFinite(deltaLines) || deltaLines === 0) return false;
		const next = Math.max(
			0,
			Math.min(this.transcriptMaxScrollOffset, this.transcriptScrollOffset + Math.trunc(deltaLines)),
		);
		if (next === this.transcriptScrollOffset) return true;
		this.transcriptScrollOffset = next;
		this.requestRender();
		return true;
	}

	scrollTranscriptPage(direction: number): boolean {
		const amount = Math.max(1, Math.floor(this.transcriptViewportHeight * 0.8));
		return this.scrollTranscript(direction * amount);
	}

	override render(width: number): string[] {
		const anchor = this.bottomAnchorStartComponent;
		if (!anchor) return super.render(width);
		const anchorIndex = this.children.indexOf(anchor);
		if (anchorIndex === -1) return super.render(width);

		const topLines: string[] = [];
		for (let i = 0; i < anchorIndex; i++) {
			topLines.push(...this.children[i]!.render(width));
		}

		const bottomLines: string[] = [];
		const fullBottomComponentRanges: BottomComponentRange[] = [];
		for (let i = anchorIndex; i < this.children.length; i++) {
			const component = this.children[i]!;
			const start = bottomLines.length;
			bottomLines.push(...component.render(width));
			fullBottomComponentRanges.push({ component, start, end: bottomLines.length });
		}

		const height = Math.max(0, this.terminal.rows);
		if (height === 0) return [...topLines, ...bottomLines];
		if (bottomLines.length >= height) {
			this.transcriptViewportHeight = 0;
			this.transcriptTopLineCount = topLines.length;
			this.transcriptAllLines = topLines;
			this.transcriptMaxScrollOffset = Math.max(0, topLines.length);
			this.bottomViewportStartRow = 0;
			const sliceStart = bottomLines.length - height;
			this.bottomComponentRanges = fullBottomComponentRanges
				.map((range) => ({
					component: range.component,
					start: Math.max(0, range.start - sliceStart),
					end: Math.min(height, range.end - sliceStart),
				}))
				.filter((range) => range.end > range.start);
			return bottomLines.slice(sliceStart);
		}

		const topViewportHeight = height - bottomLines.length;
		this.transcriptViewportHeight = topViewportHeight;
		if (
			this.transcriptScrollOffset > 0 &&
			this.transcriptTopLineCount > 0 &&
			topLines.length > this.transcriptTopLineCount
		) {
			this.transcriptScrollOffset += topLines.length - this.transcriptTopLineCount;
		}
		this.transcriptTopLineCount = topLines.length;
		this.transcriptAllLines = topLines;
		this.transcriptMaxScrollOffset = Math.max(0, topLines.length - topViewportHeight);
		this.transcriptScrollOffset = Math.max(0, Math.min(this.transcriptScrollOffset, this.transcriptMaxScrollOffset));

		const start = Math.max(0, topLines.length - topViewportHeight - this.transcriptScrollOffset);
		const visibleTopLines = topLines.slice(start, start + topViewportHeight);
		this.transcriptVisibleStartLine = start;
		this.bottomViewportStartRow = topViewportHeight;
		this.bottomComponentRanges = fullBottomComponentRanges;
		const spacerLines = Math.max(0, topViewportHeight - visibleTopLines.length);
		return [...this.highlightTranscriptSelection(visibleTopLines), ...Array(spacerLines).fill(""), ...bottomLines];
	}

	private transcriptPoint(row: number, col: number, clamp = false): { line: number; col: number } | undefined {
		if (this.transcriptViewportHeight <= 0) return undefined;
		if (!clamp && (row < 0 || row >= this.transcriptViewportHeight)) return undefined;
		const clampedRow = Math.max(0, Math.min(this.transcriptViewportHeight - 1, row));
	...
	private orderedTranscriptSelection():
		...
		return anchor.line < focus.line || (anchor.line === focus.line && anchor.col <= focus.col)
			? { start: anchor, end: focus }
			: { start: focus, end: anchor };
	}

	private highlightTranscriptSelection(lines: string[]): string[] {
		const selection = this.orderedTranscriptSelection();
		if (!selection) return lines;
		return lines.map((line, index) => {
			const logicalLine = this.transcriptVisibleStartLine + index;
			if (logicalLine < selection.start.line || logicalLine > selection.end.line) return line;
			const lineWidth = Math.max(visibleWidth(line), this.terminal.columns);
			const start = logicalLine === selection.start.line ? selection.start.col : 0;
			const end = logicalLine === selection.end.line ? selection.end.col : lineWidth;
			if (end <= start) return line;
			return `${sliceByColumn(line, 0, start, true)}\x1b[7m${sliceByColumn(line, start, end - start, true)}\x1b[27m${sliceByColumn(line, end, lineWidth - end, true)}`;
		});
	}

	private clearTranscriptSelection(): boolean {
		this.stopTranscriptSelectionAutoscroll();
		if (!this.transcriptSelection) return false;
		this.transcriptSelection = undefined;
		if (this.selectionAttention === "transcript") this.selectionAttention = null;
		this.requestRender();
		return true;
	}

	private copyTranscriptSelection(): boolean {
		const selection = this.orderedTranscriptSelection();
		if (!selection) return false;
		const copied = [] as string[];
		for (let line = selection.start.line; line <= selection.end.line; line++) {
			const text = this.transcriptAllLines[line] ?? "";
			const width = Math.max(visibleWidth(text), this.terminal.columns);
			const start = line === selection.start.line ? selection.start.col : 0;
			const end = line === selection.end.line ? selection.end.col : width;
			copied.push(this.stripAnsi(sliceByColumn(text, start, Math.max(0, end - start), true)).replace(/\s+$/g, ""));
		}
	...
	private setSelectionAttention(scope: "transcript" | "bottom-component" | null, except?: Component): void {
		if (scope !== "transcript") this.clearTranscriptSelection();
		if (scope !== "bottom-component") {
			this.activeBottomSelectionComponent = null;
			this.activeBottomSelectionTarget = null;
		}
		for (const range of this.bottomComponentRanges) {
			this.visitBottomComponents(range.component, (component) => {
				if (component !== except) component.clearSelection?.();
			});
		}
		this.selectionAttention = scope;
	}

	private visitBottomComponents(
		component: Component,
		visitor: (component: Component) => void,
		seen = new Set<Component>(),
	): void {
		if (seen.has(component)) return;
		seen.add(component);
		visitor(component);
		if (component instanceof Container) {
			for (const child of component.children) this.visitBottomComponents(child, visitor, seen);
		}
	}

	private componentRenderedHeight(component: Component): number {
		try {
			return Math.max(1, component.render(this.terminal.columns).length);
		} catch {
			return 1;
		}
	}

	private findBottomSelectionTarget(
		component: Component,
		localRow: number,
		col: number,
		screenStartRow: number,
	): BottomSelectionTarget | undefined {
		const height = this.componentRenderedHeight(component);
		if (component.beginMouseSelection) {
			return {
				component,
				row: Math.max(0, Math.min(height - 1, Math.trunc(localRow))),
				col,
				screenStartRow,
				height,
			};
		...
		for (const child of component.children) {
			const childHeight = this.componentRenderedHeight(child);
			if (localRow >= offset && localRow < offset + childHeight) {
				return this.findBottomSelectionTarget(child, localRow - offset, col, screenStartRow + offset);
			}
			offset += childHeight;
		}
		return undefined;
	...
	private bottomSelectionTarget(row: number, col: number, active?: Component): BottomSelectionTarget | undefined {
		if (active && this.activeBottomSelectionTarget?.component === active) {
			const target = this.activeBottomSelectionTarget;
			return {
				...target,
				row: Math.max(0, Math.min(target.height - 1, Math.trunc(row - target.screenStartRow))),
				col,
			};
		}
		const relativeRow = row - this.bottomViewportStartRow;
		const range = this.bottomComponentRanges.find((item) => relativeRow >= item.start && relativeRow < item.end);
		if (!range) return undefined;
		return this.findBottomSelectionTarget(
			range.component,
			relativeRow - range.start,
			col,
			this.bottomViewportStartRow + range.start,
		);
	}

	private updateTranscriptSelectionAutoscroll(row: number): void {
		if (!this.transcriptSelection?.dragging) {
			this.stopTranscriptSelectionAutoscroll();
			return;
		}
		if (row <= 0) this.startTranscriptSelectionAutoscroll(-1);
		else if (row >= this.transcriptViewportHeight - 1) this.startTranscriptSelectionAutoscroll(1);
		else this.stopTranscriptSelectionAutoscroll();
	}

	private startTranscriptSelectionAutoscroll(direction: -1 | 1): void {
		const canScroll =
			direction < 0 ? this.transcriptScrollOffset < this.transcriptMaxScrollOffset : this.transcriptScrollOffset > 0;
		if (!canScroll) {
			this.stopTranscriptSelectionAutoscroll();
			return;
		}
		if (this.transcriptSelectionAutoscrollDirection === direction && this.transcriptSelectionAutoscrollTimer) return;
		this.stopTranscriptSelectionAutoscroll();
		this.transcriptSelectionAutoscrollDirection = direction;
		this.transcriptSelectionAutoscrollTimer = setInterval(() => {
			if (!this.transcriptSelection?.dragging) {
				this.stopTranscriptSelectionAutoscroll();
				return;
			}
			const before = this.transcriptScrollOffset;
			this.scrollTranscript(direction < 0 ? 1 : -1);
			if (before === this.transcriptScrollOffset) {
				this.stopTranscriptSelectionAutoscroll();
				return;
			}
			this.transcriptSelection.focus.line += direction < 0 ? -1 : 1;
		}, 120);
	}

	private stopTranscriptSelectionAutoscroll(): void {
		if (this.transcriptSelectionAutoscrollTimer) clearInterval(this.transcriptSelectionAutoscrollTimer);
		this.transcriptSelectionAutoscrollTimer = undefined;
		this.transcriptSelectionAutoscrollDirection = 0;
	}

	private consumeTranscriptSelectionInput(data: string): boolean {
		if (!this.bottomAnchorStartComponent || this.hasOverlay()) return false;
		const event = data.match(/^\x1b\[<(\d+);(\d+);(\d+)([mM])$/);
		if (event) {
			const code = Number.parseInt(event[1]!, 10);
			if ((code & 64) !== 0) return false;
			const row = Number.parseInt(event[3]!, 10) - 1;
			const col = Number.parseInt(event[2]!, 10) - 1;
			const isMotion = (code & 32) !== 0;
			if (event[4] === "M" && (code & 3) === 0 && !isMotion) {
				const point = this.transcriptPoint(row, col);
				if (point) {
					this.setSelectionAttention("transcript");
					this.transcriptSelection = { anchor: point, focus: point, dragging: true };
					this.updateTranscriptSelectionAutoscroll(row);
					this.requestRender();
					return true;
				}
				const target = this.bottomSelectionTarget(row, col);
				if (!target) {
					this.setSelectionAttention(null);
					return false;
				}
				this.setSelectionAttention("bottom-component", target.component);
				this.activeBottomSelectionComponent = target.component;
				this.activeBottomSelectionTarget = target;
				target.component.beginMouseSelection?.(target.row, target.col);
				this.requestRender();
				return true;
			}
			if (
				event[4] === "M" &&
				isMotion &&
				this.selectionAttention === "transcript" &&
				this.transcriptSelection?.dragging
			) {
				const point = this.transcriptPoint(row, col, true);
				if (point) this.transcriptSelection.focus = point;
				this.updateTranscriptSelectionAutoscroll(row);
				this.requestRender();
				return true;
			}
			if (
				event[4] === "M" &&
				isMotion &&
				this.selectionAttention === "bottom-component" &&
				this.activeBottomSelectionComponent
			) {
				const target = this.bottomSelectionTarget(row, col, this.activeBottomSelectionComponent);
				if (target?.component.updateMouseSelection) {
					target.component.updateMouseSelection(target.row, target.col);
					this.requestRender();
					return true;
				}
			}
			if (event[4] === "m" && this.selectionAttention === "transcript" && this.transcriptSelection?.dragging) {
				const point = this.transcriptPoint(row, col, true);
				if (point) this.transcriptSelection.focus = point;
				this.transcriptSelection.dragging = false;
				this.stopTranscriptSelectionAutoscroll();
				this.requestRender();
				return true;
			}
			if (
				...
				const target = this.bottomSelectionTarget(row, col, this.activeBottomSelectionComponent);
				target?.component.endMouseSelection?.(target.row, target.col);
				this.activeBottomSelectionComponent = null;
				this.activeBottomSelectionTarget = null;
				this.requestRender();
				return true;
		...
		if (this.selectionAttention !== "transcript" || !this.transcriptSelection) return false;
		const keybindings = getKeybindings();
		if (keybindings.matches(data, "tui.input.copySelection") || keybindings.matches(data, "tui.input.copy")) {
			return this.copyTranscriptSelection();
		}
		if (matchesKey(data, "escape")) return this.clearTranscriptSelection();
		return false;
	}

	private consumeTranscriptScrollInput(data: string): boolean {
		if (!this.bottomAnchorStartComponent || this.hasOverlay()) return false;
		if (/^\x1b\[<64;\d+;\d+[mM]$/.test(data)) return this.scrollTranscript(3);
		if (/^\x1b\[<65;\d+;\d+[mM]$/.test(data)) return this.scrollTranscript(-3);
		const routeLegacyWheelArrows =
			process.env.PI_TUI_NATIVE_SELECTION === "1" && process.env.PI_TUI_EDITOR_SCROLL !== "1";
		if (matchesKey(data, "alt+up") || (routeLegacyWheelArrows && (data === "\x1b[A" || data === "\x1bOA"))) {
			return this.scrollTranscript(1);
		}
		if (matchesKey(data, "alt+down") || (routeLegacyWheelArrows && (data === "\x1b[B" || data === "\x1bOB"))) {
			return this.scrollTranscript(-1);
		}
		const keybindings = getKeybindings();
		if (keybindings.matches(data, "tui.editor.pageUp")) return this.scrollTranscriptPage(1);
		if (keybindings.matches(data, "tui.editor.pageDown")) return this.scrollTranscriptPage(-1);
		return false;
	}
