...
export class Editor implements Component, Focusable {
	...
	private selectionFocus: { line: number; col: number } | undefined;
	private selectionKind: "keyboard" | "mouse" | undefined;
	...
	protected lastVisibleLayoutLines: LayoutLine[] = [];
	protected lastPaddingX = 0;
	protected lastTextRowStart = 1;
	...
	protected segment(text: string, mode: "word" | "grapheme"): Iterable<Intl.SegmentData> {
	...
	render(width: number): string[] {
		...
		for (const layoutLine of visibleLines) {
			...
			let displayText = this.styleSelectionInLayoutLine(
				...
				hasSelection && emitCursorMarker && layoutLine.hasCursor,
			);
			let lineVisibleWidth = visibleWidth(layoutLine.text);
			let cursorInPadding = false;

			// Add cursor if this line has it
			if (!hasSelection && layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
	...
	handleInput(data: string): void {
		...
		if (kb.matches(data, "tui.input.copySelection")) {
			this.copySelection();
			return;
		}
		if (kb.matches(data, "tui.editor.selectUp")) {
			this.extendKeyboardSelection(-1, 0);
			return;
		}
		if (kb.matches(data, "tui.editor.selectDown")) {
			this.extendKeyboardSelection(1, 0);
			return;
		}
		if (kb.matches(data, "tui.editor.selectLeft")) {
			this.extendKeyboardSelection(0, -1);
			return;
		}
		if (kb.matches(data, "tui.editor.selectRight")) {
			this.extendKeyboardSelection(0, 1);
			return;
		}
	...
	protected layoutText(contentWidth: number): LayoutLine[] {
	...
	private normalizePosition(position: { line: number; col: number }): { line: number; col: number } {
		const line = Math.max(0, Math.min(this.state.lines.length - 1, Math.trunc(position.line)));
		const maxCol = this.state.lines[line]?.length ?? 0;
		return { line, col: Math.max(0, Math.min(maxCol, Math.trunc(position.col))) };
	}

	protected getOrderedSelection():
		| { start: { line: number; col: number }; end: { line: number; col: number } }
		| undefined {
		if (!this.selectionAnchor || !this.selectionFocus) return undefined;
		const anchor = this.normalizePosition(this.selectionAnchor);
		const focus = this.normalizePosition(this.selectionFocus);
		if (anchor.line === focus.line && anchor.col === focus.col) return undefined;
		return anchor.line < focus.line || (anchor.line === focus.line && anchor.col <= focus.col)
			? { start: anchor, end: focus }
			: { start: focus, end: anchor };
	}

	protected styleSelectionInLayoutLine(
		text: string,
		logicalLine: number,
		startCol: number,
		endCol: number,
		...
		const marker = emitCursorMarker && cursorPos !== undefined ? CURSOR_MARKER : "";
		if (!selection || logicalLine < selection.start.line || logicalLine > selection.end.line) {
			return marker && cursorPos !== undefined
				? `${text.slice(0, cursorPos)}${marker}${text.slice(cursorPos)}`
				: text;
		}
		const absoluteStart = logicalLine === selection.start.line ? selection.start.col : startCol;
		const absoluteEnd = logicalLine === selection.end.line ? selection.end.col : endCol;
		const from = Math.max(0, Math.min(text.length, absoluteStart - startCol));
		const to = Math.max(from, Math.min(text.length, absoluteEnd - startCol));
		if (to <= from) {
			return marker && cursorPos !== undefined
				? `${text.slice(0, cursorPos)}${marker}${text.slice(cursorPos)}`
				: text;
		}
		const insertMarker = (offset: number): string => (marker && cursorPos === offset ? marker : "");
		return (
			`${insertMarker(0)}${text.slice(0, from)}` +
			`${insertMarker(from)}\x1b[7m${text.slice(from, to)}${insertMarker(to)}\x1b[27m` +
			`${text.slice(to)}${insertMarker(text.length)}`
		);
	}
	...
	private copySelection(): boolean {
		...
		const text = this.state.lines
			...
			.map((line, index) => {
				const lineNo = selection.start.line + index;
				const start = lineNo === selection.start.line ? selection.start.col : 0;
				const end = lineNo === selection.end.line ? selection.end.col : line.length;
	...
	private deleteSelectionBeforeMutation(): boolean {
		const selection = this.getOrderedSelection();
		if (!selection) return false;
		const startLine = this.state.lines[selection.start.line] ?? "";
		const endLine = this.state.lines[selection.end.line] ?? "";
		const replacement = startLine.slice(0, selection.start.col) + endLine.slice(selection.end.col);
		this.state.lines.splice(selection.start.line, selection.end.line - selection.start.line + 1, replacement);
	...
	private stringOffsetForColumn(text: string, column: number): number {
		const target = Math.max(0, Math.trunc(column));
		let visual = 0;
		for (const segment of this.segment(text, "grapheme")) {
			const width = Math.max(1, visibleWidth(segment.segment));
			if (visual + width > target) return segment.index;
			visual += width;
		}
		return text.length;
	}

	private pointForRenderedCell(row: number, col: number): { line: number; col: number } {
		if (this.lastVisibleLayoutLines.length === 0) return this.cursorPosition();
		const visualIndex = Math.max(
			0,
			Math.min(this.lastVisibleLayoutLines.length - 1, Math.trunc(row) - this.lastTextRowStart),
		);
		const layoutLine = this.lastVisibleLayoutLines[visualIndex];
		if (!layoutLine) return this.cursorPosition();
		const rawColumn = Math.max(0, Math.trunc(col) - this.lastPaddingX);
		const offset = this.stringOffsetForColumn(layoutLine.text, rawColumn);
		return this.normalizePosition({ line: layoutLine.logicalLine, col: layoutLine.startCol + offset });
	}

	beginMouseSelection(row: number, col: number): boolean {
		const point = this.pointForRenderedCell(row, col);
		this.selectionAnchor = point;
		this.selectionFocus = point;
		this.selectionKind = "mouse";
		this.tui.requestRender();
		return true;
	}

	updateMouseSelection(row: number, col: number): boolean {
		if (this.selectionKind !== "mouse" || !this.selectionAnchor) return false;
		this.selectionFocus = this.pointForRenderedCell(row, col);
		this.tui.requestRender();
		return true;
	}
	...
	private extendKeyboardSelection(deltaLine: number, deltaCol: number): void {
		if (this.selectionKind !== "keyboard" || !this.selectionAnchor || !this.selectionFocus) {
			const cursor = this.cursorPosition();
			this.selectionAnchor = cursor;
			this.selectionFocus = cursor;
			this.selectionKind = "keyboard";
		}
		const focus = this.normalizePosition(this.selectionFocus);
		this.state.cursorLine = focus.line;
		this.state.cursorCol = focus.col;
		this.preferredVisualCol = null;
		this.snappedFromCursorCol = null;
		this.moveCursor(deltaLine, deltaCol);
		this.selectionFocus = this.cursorPosition();
		this.tui.requestRender();
	}
	...
	private insertTextAtCursorInternal(text: string): void {
		if (!text) return;
		this.deleteSelectionBeforeMutation();
	...
	private insertCharacter(char: string, skipUndoCoalescing?: boolean): void {
		this.exitHistoryBrowsing();
		const replacesSelection = this.getOrderedSelection() !== undefined;
		...
		if (!skipUndoCoalescing) {
			if (replacesSelection || isWhitespaceChar(char) || this.lastAction !== "type-word") {
				this.pushUndoSnapshot();
			}
