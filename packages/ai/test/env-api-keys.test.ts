...
const originalAdRouterApiKey = process.env.ADROUTER_API_KEY;
...
afterEach(() => {
	...
	if (originalAdRouterApiKey === undefined) {
		delete process.env.ADROUTER_API_KEY;
	} else {
		process.env.ADROUTER_API_KEY = originalAdRouterApiKey;
	}
});
...
describe("environment API keys", () => {
	...
	it("resolves AdRouter credentials from ADROUTER_API_KEY", () => {
		process.env.ADROUTER_API_KEY = "adrouter-token";
		expect(findEnvKeys("adrouter")).toEqual(["ADROUTER_API_KEY"]);
		expect(getEnvApiKey("adrouter")).toBe("adrouter-token");
	});
});
