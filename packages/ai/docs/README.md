...
The [canonical AdRouterCLI product guide](https://github.com/adrouter/adrouterCLI/blob/main/docs/about.md)
documents the generated hosted catalog and the boundary between Router and CLI responsibilities.
...
```bash
npm install @adrouter/ai
```

TypeBox exports are re-exported from `@adrouter/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@adrouter/ai';
import { builtinModels } from '@adrouter/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@adrouter/ai/providers/anthropic';
import { openaiProvider } from '@adrouter/ai/providers/openai';
import { openrouterProvider } from '@adrouter/ai/providers/openrouter';
import { amazonBedrockProvider } from '@adrouter/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@adrouter/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@adrouter/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@adrouter/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@adrouter/ai';
...
```typescript
import { validateToolCall, type Tool } from '@adrouter/ai';
...
```typescript
import { builtinImagesModels } from '@adrouter/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@adrouter/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@adrouter/ai';
import { openAICompletionsApi } from '@adrouter/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@adrouter/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@adrouter/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@adrouter/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@adrouter/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@adrouter/ai';
import { anthropicProvider } from '@adrouter/ai/providers/anthropic';
import { openaiProvider } from '@adrouter/ai/providers/openai';
import { googleProvider } from '@adrouter/ai/providers/google';
...
```typescript
import { createModels } from '@adrouter/ai';
import { anthropicProvider } from '@adrouter/ai/providers/anthropic';
...
- `@adrouter/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@adrouter/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@adrouter/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@adrouter/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@adrouter/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@adrouter/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@adrouter/ai/bedrock-provider';
...
```bash
npx @adrouter/ai login              # interactive provider selection
npx @adrouter/ai login anthropic    # login to specific provider
npx @adrouter/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@adrouter/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@adrouter/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@adrouter/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@adrouter/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@adrouter/ai`.
