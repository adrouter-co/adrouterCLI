#!/usr/bin/env bash
#
# Build AdRouterCLI binaries for all platforms locally.
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
		bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/adrouter-bin.exe"
		cp scripts/adrouter-bun-launcher.cmd "$OUTPUT_DIR/$platform/adrouter.cmd"
    else
		bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/adrouter-bin"
		cp scripts/adrouter-bun-launcher.sh "$OUTPUT_DIR/$platform/adrouter"
		chmod +x "$OUTPUT_DIR/$platform/adrouter"
    fi
done

echo "==> Creating release archives..."

cmp BUNDLED_SOURCES.json ../../docs/bundled-sources.json

# Copy shared files to each platform directory
for platform in "${PLATFORMS[@]}"; do
    cp package.json "$OUTPUT_DIR/$platform/"
	cp BUNDLED_SOURCES.json "$OUTPUT_DIR/$platform/"
	cp THIRD_PARTY_NOTICES.md "$OUTPUT_DIR/$platform/"
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
		echo "Creating adrouter-$platform.zip..."
		(cd "$platform" && zip -r ../adrouter-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
		echo "Creating adrouter-$platform.tar.gz..."
		mv "$platform" adrouter && tar -czf adrouter-$platform.tar.gz adrouter && mv adrouter "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
		mkdir -p "$platform" && (cd "$platform" && unzip -q ../adrouter-$platform.zip)
    else
		tar -xzf adrouter-$platform.tar.gz && mv adrouter "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
	if [[ "$platform" == windows-* ]]; then
		echo "  $OUTPUT_DIR/$platform/adrouter.cmd"
    else
		echo "  $OUTPUT_DIR/$platform/adrouter"
    fi
done
