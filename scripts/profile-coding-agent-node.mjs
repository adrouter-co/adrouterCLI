...
const agentDirEnvName = "ADROUTER_CODING_AGENT_DIR";
...
Options:
  ...
  --agent-dir <dir>      Use a specific ADROUTER_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force the compatibility offline/version-check environment flags
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "adrouter-startup-benchmark-")) : undefined;
