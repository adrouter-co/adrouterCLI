...
## Beta.6 candidate and promotion

Tag beta.6 only after the release commit and six-platform CI are green:
...
2. `finalize-release` re-verifies the candidate, installs the exact version
   ...
   `latest` to beta.6, removes `candidate`, deprecates beta.5, and makes the
...
After beta.6 has run for at least 48 hours without a release blocker, require
...
only. If any runtime code changes after beta.6, publish beta.7 and restart the
48-hour soak.
...
- `finalTags.beta` remains `0.81.0-beta.6`;
...
gates.
Promotion moves only `latest` to stable, leaves `beta` on beta.6, removes
