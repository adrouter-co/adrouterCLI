...
Run `/login adrouter`, sign in at the printed AdRouter website, and type `DONE` only after the website confirms you are signed in. The CLI then creates a fresh installation key and opens the matching WebUI approval page; an already-open signed-in tab also shows the approval modal. Open the printed URL manually if the browser does not launch. Failed attempts remove the pending key and best-effort cancel server state, so a later `/login adrouter` starts cleanly. The device code, private key, and credentials are never displayed. Then select `agnes-2.0-flash`, `agnes-2.5-flash`, `agnes-2.5-pro`, `agnes-2.5-pro-alpha`, `deepseek-v4-flash`, `deepseek-v4-pro`, `mimo-v2.5`, or `mimo-v2.5-pro`. For non-interactive work after enrollment:
